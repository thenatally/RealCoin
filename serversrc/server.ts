...
async function savePortfolioSnapshot(userId: string, portfolio: any) {
	try {
		const portfolioWithGains = marketEngine.getPortfolioWithGains(portfolio, marketEngine.coins);
		
		// Calculate holdings value
		const holdingsValue = Object.entries(portfolioWithGains.holdings).reduce((sum, [coinId, holding]: [string, any]) => {
			return sum + holding.totalValue;
		}, 0);

		// Create portfolio history entry
		const historyEntry: PortfolioHistoryData = {
			userId,
			timestamp: new Date(),
			totalValue: portfolioWithGains.totalValue,
			holdingsValue,
			cashValue: portfolioWithGains.cash,
			holdings: Object.entries(portfolioWithGains.holdings).reduce((acc, [coinId, holding]: [string, any]) => {
				acc[coinId] = {
				...
				return acc;
			}, {} as Record<string, { amount: number; value: number; avgCost: number }>)
		};
...
async function cleanupPortfolioHistory() {
	try {
		...
		// Group by user
		const userHistories = new Map<string, string[]>();
		...
		for (const [userId, historyIds] of userHistories) {
			...
			const entries: Array<{ id: string, entry: PortfolioHistoryData }> = [];
			...
			if (olderEntries.length > 0) {
				...
				const significantEntries = new Set<string>();
				...
				const timeBasedKeepers = new Set<string>();
	...
	switch (pathname) {
		...
		case '/api/trading/portfolio-history':
			if (req.method === 'GET') {
				try {
					const url = new URL(req.url!, `http://${req.headers.host}`);
					const timeframe = url.searchParams.get('timeframe') as '5min' | '10min' | '30min' | '1hour' | '4hour' | '8hour' | '1day' | '4day' | '1week' | '1month' || '1day';
					...
					let historyEntries: PortfolioHistoryData[] = [];
					...
					if (historyEntries.length > limit && limit > 0) {
						...
						const downsampledEntries: PortfolioHistoryData[] = [];
