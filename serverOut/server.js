...
function broadcastToAllRooms(message, excludeWs) {
...
function decryptSecret(encrypted) {
    ...
    const decipher = crypto.createDecipheriv(ALGO, Buffer.from(SECRET_KEY, 'hex'), Buffer.from(ivHex, 'hex'));
...
const accountDB = new DBMap('accounts', z.object({
...
const tokenDB = new DBMap('tokens', z.object({
    userId: z.string(),
    createdAt: z.date(),
    expiresAt: z.date(),
    pendingTOTP: z
...
class PriceHistoryManager {
    ...
    async getPriceHistory(coinId, timeframe, limit = 60) {
        try {
...
class Bot {
    ...
    async momentumMaxineTrade(marketState) {
        ...
        if (!coin)
            return;
        if (!this.parameters.priceHistory)
            this.parameters.priceHistory = [];
        this.parameters.priceHistory.push(coin.price);
        if (this.parameters.priceHistory.length > 10)
            this.parameters.priceHistory.shift();
    ...
    async meanRevertorMarvinTrade(marketState) {
        ...
        if (!this.parameters.priceHistory)
            this.parameters.priceHistory = [];
        this.parameters.priceHistory.push(coin.price);
        if (this.parameters.priceHistory.length > 20)
            this.parameters.priceHistory.shift();
        if (this.parameters.priceHistory.length < 10)
            return;
    ...
    async patternProphetTrade(marketState) {
        ...
        if (!this.parameters.priceHistory)
            this.parameters.priceHistory = [];
        this.parameters.priceHistory.push(coin.price);
        if (this.parameters.priceHistory.length > 5)
            this.parameters.priceHistory.shift();
        if (this.parameters.priceHistory.length < 3)
            return;
    ...
    async makeTrade(marketState, coin, side, amount) {
    ...
    getBestPerformingCoin(marketState) {
        let bestCoin = null;
    ...
    getWorstPerformingCoin(marketState) {
        let worstCoin = null;
    ...
    updateMarketHistory(marketState) {
    ...
    async technicalTedTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
    ...
    async fundamentalFrankTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin || Math.random() > 0.01)
            return;
        if (coin.liquidity > 2000 && coin.baseVol < 0.02) {
            await this.makeTrade(marketState, coin, 'buy', this.getTradeSize(coin, 'moderate'));
        }
    }
    async randomRickTrade(marketState) {
        if (Math.random() < 0.1) {
            const coin = marketState.getCoin(this.targetCoin);
            if (!coin)
                return;
    ...
    async correlationCoraTrade(marketState) {
        if (this.watchedCoins.length < 2)
            return;
    ...
    async volatilityVictorTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
    ...
    async liquidityLucyTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin || coin.liquidity < 1000)
            return;
    ...
    async breakoutBobTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
        const history = this.parameters.marketHistory?.[this.targetCoin] || [];
    ...
    async supportSarahTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
        const history = this.parameters.marketHistory?.[this.targetCoin] || [];
    ...
    async marketmakerMikeTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
    ...
    async sniperSteveTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
        const pctChange = this.getPriceChangePercent(coin, 5);
    ...
    async panicPeteTrade(marketState) {
    ...
    async trendfollowerTimTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
    ...
    async meanreversionMaryTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
    ...
    async fibonacciFranTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
        const history = this.parameters.marketHistory?.[this.targetCoin] || [];
    ...
    async volumeVinceTrade(marketState) {
        const coin = marketState.getCoin(this.targetCoin);
        if (!coin)
            return;
        const pctChange = this.getPriceChangePercent(coin, 5);
...
class MarketAnalyticsEngine {
    ...
    calculateAnalytics(coins, priceHistory, trades) {
        const coinAnalytics = [];
...
class MarketEventsSystem {
    ...
    generateRandomEvent(coins) {
        ...
        const severities = ['minor', 'moderate', 'major', 'extreme'];
    ...
    applyEventEffects(coins, priceHistoryManager) {
        const now = new Date();
        const expiredEvents = [];
...
export class MarketEngine {
    ...
    async initializeDefaultBots() {
        ...
        for (let i = 1; i <= 150; i++) {
            ...
            else if (personality === 'longterm-larry')
                cash = 5000 + Math.random() * 10000;
            else if (personality === 'influencer-izzy')
                cash = 10000 + Math.random() * 20000;
            else if (personality === 'quant-quinn')
                cash = 3000 + Math.random() * 7000;
            else
                cash = 800 + Math.random() * 2000;
            defaultBots.push({
    ...
    async updatePortfolio(userId, coinId, side, amount, price) {
...
function generateBackupCodes(count = 10, length = 10) {
    return Array.from({ length: count }, () => crypto
        .randomBytes(length)
        .toString('base64')
        .replace(/[^a-zA-Z0-9]/g, '')
        .slice(0, length));
...
async function handleInternalHTTP(req, res) {
                ...
                if (req.method === 'POST') {
                    ...
                    try {
                        ...
                        const editSchema = z.object({
                            token: z.string(),
                            displayName: z.string().min(1).max(64).optional(),
                            avatarUrl: z.preprocess((val) => (val === '' ? undefined : val), z.string().url().optional())
                        });
        ...
        case '/internal/accounts/finalize-totp-setup':
            {
                if (req.method === 'POST') {
                    ...
                    try {
                        ...
                        let storedBackupHashes = pending.backupCodes
                        ...
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ error: 'Invalid JSON or server error', detail: String(error) }));
                        return;
                    }
                }
            }
    ...
    switch (pathname) {
        ...
        case '/api/trading/analytics':
            if (req.method === 'GET') {
                try {
                    const recentTrades = globalTradeTracker.getRecentTrades(1000);
                    const analytics = marketEngine.analytics.calculateAnalytics(marketEngine.coins, marketEngine.priceHistoryManager, recentTrades);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            if (req.url?.startsWith('/api/trading/user/') && req.method === 'GET') {
                try {
                    ...
                    const userTrades = allTrades
                        .filter((trade) => trade.buyerId === username || trade.sellerId === username)
